...
function address(interfaceName, callback) {
  ...
  address.mac(interfaceName, function (err, mac) {
    ...
    // return addr: { ip: IPv4, ipv6: IPV6, mac }
    callback(err, addr);
  });
}
...
address.interface = function (family, name) {
  ...
  family = family || 'IPv4'; // default using IPv4
  for (var i = -1; i < 8; i++) {
    // 'en0' ~ 'en8'
 ...
 * 
 * address.ip() => address.interface('IPv4') => '192.168.0.2'
 */
 ...
 * @param {String} [interfaceName] interface name, default is 'eth' on linux, 'en' on mac os.
 * @return {String} IP address
 * address.ip() => address.interface('IPv6') => 'fe80::7aca:39ff:feb0:e67d'
 */
